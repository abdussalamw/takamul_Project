...
<form method="POST" class="program-form" id="<?php echo htmlspecialchars($options['form_id']); ?>" enctype="multipart/form-data" action="<?php echo htmlspecialchars($options['form_action']); ?>">
    ...
    <!-- Admin: Dropdown select for organizer -->
    <div class="form-group" style="grid-column: span 2;">
        <label for="organizer_id">
            <i class="fas fa-building"></i> <?php echo $field_labels['organizer_id']; ?> <span class="required-asterisk">*</span>
        </label>
        <select id="organizer_id" name="organizer_id" required>
            <option value="">اختر الجهة...</option>
            <option value="new" <?php echo getFormValueSelected($form_data, 'organizer_id', 'new'); ?>>
                جهة جديدة (إدخال بيانات جهة جديدة)
            </option>
            <?php foreach ($orgs_data as $org): ?>
                <option value="<?php echo $org['id']; ?>" <?php echo getFormValueSelected($form_data, 'organizer_id', $org['id']); ?>>
    ...
    <?php else: ?>
    <!-- Public: Simple text input for organizer name -->
    <div class="form-group" style="grid-column: span 2;">
        ...
        <input type="hidden" name="organizer_id" value="new">
    </div>
    <?php endif; ?>

    <?php if ($options['is_admin']): ?>
    <div class="form-group half-width" id="organizer_name_group" style="display: none;">
        <label for="organizer_name">
            <i class="fas fa-building"></i> <?php echo $field_labels['organizer_name']; ?> <span class="required-asterisk">*</span>
        </label>
        <input type="text" id="organizer_name" name="organizer_name" 
               value="<?php echo getFormValue($form_data, 'organizer_name'); ?>" 
               placeholder="الجمعية أو المؤسسة المظلة">
    </div>
    <?php endif; ?>
    ...
    <div class="form-group" id="entry_officer_phone_group" style="grid-column: span 2;">
        ...
        <span class="form-note">نرجو كتابة الرقم بصيغة 9665**********</span>
    </div>
